...
import lucid.livekit as livekit_module
from lucid.livekit import OutputRouter, RealLiveKitAdapter
...
async def test_output_router_passes_video_frames_by_reference() -> None:
    ...
    router = OutputRouter(
        outputs=(publish.video(name="video", width=2, height=2, fps=30),),
        livekit=adapter,
        target_fps=30,
        max_queue_frames=4,
        frame_width=2,
    ...
    metrics = router.metrics(inference_ms_p50=12.5)
    assert metrics.queue_depth == 0
    assert metrics.publish_dropped_frames == 0
    assert metrics.inference_ms_p50 == 12.5
...
async def test_output_router_paces_publish_inline(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    adapter = _StubLiveKitAdapter()
    router = OutputRouter(
        outputs=(publish.video(name="video", width=2, height=2, fps=60),),
        livekit=adapter,
        target_fps=10,
        max_queue_frames=4,
        frame_width=2,
    ...
    assert publish_times == [100.0, 100.1]
