...
def test_modal_dispatcher_cancel_passes_force(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "lucid.modal.app.modal.FunctionCall.from_id",
...
def test_with_lucid_runtime_uses_absolute_package_and_extra_dirs(
    ...
    assert image.local_dirs[1] == (str(extra_dir.resolve()), "/workspace/demo")
    assert image.commands == [
        "python -m pip install '/workspace/packages/lucid[livekit,modal]'",
    ]
