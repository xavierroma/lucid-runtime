...
from lucid.controlplane import CoordinatorAuthError, CoordinatorClient
...
async def test_coordinator_client_posts_internal_session_paths() -> None:
    ...
    await client.ready("session-1")
    await client.running("session-1")
    await client.paused("session-1")
    await client.heartbeat("session-1")
    await client.ended("session-1", "MODEL_RUNTIME_ERROR", "WORKER_REPORTED_ERROR")
    await client.close()
...
@pytest.mark.asyncio
async def test_running_raises_auth_error() -> None:
