export function manifestForModel(modelId, capabilitiesManifest, staticManifests) {
...
export function findInputByName(inputs, name) {
    return inputs.find((input) => input.name === name) ?? null;
}
export function findInputsByBinding(inputs, kind) {
    return inputs.filter((input) => input.binding?.kind === kind);
}
export function findPromptInput(inputs) {
...
export function findUploadedFileInput(inputs) {
    return findUploadedFileInputs(inputs)[0] ?? null;
}
export function findUploadedFileInputs(inputs) {
    const uploaded = [];
...
export function uploadAcceptAttribute(upload) {
    return upload.mime_types.join(",");
}
export function inputProperties(input) {
    const schema = input.args_schema;
    return schema.properties ?? {};
}
export function isStringWireField(schema) {
    if (!schema) {
        return false;
    }
...
export function parseUploadField(value) {
    if (!value || typeof value !== "object") {
        return null;
    }
    const upload = value;
    const kind = typeof upload.kind === "string" ? upload.kind : null;
    const maxBytes = typeof upload.max_bytes === "number" ? upload.max_bytes : Number(upload.max_bytes);
    ...
    const targetWidth = typeof upload.target_width === "number"
        ? upload.target_width
        : Number(upload.target_width);
    const targetHeight = typeof upload.target_height === "number"
